...
import LanguageDropdown from "@/components/navigation-menu/dropdown-language";
import ProfileDropdown from "@/components/navigation-menu/dropdown-profile";
import NotificationDropdown from "@/components/navigation-menu/notification-dropdown";
...
export function AppHeader({ pageTitle }: AppHeaderProps) {
  return (
    <header className={`${styles.header} bg-card sticky top-0 z-50 border-b`}>
      <div className="mx-auto flex items-center justify-between gap-6 px-4 py-2.5 sm:px-6 ">
        <div className="flex items-center gap-2">
          <SidebarTrigger className="touch-manipulation [&_svg]:size-6 cursor-pointer" />
          <Separator
            orientation="vertical"
            className="hidden h-4! sm:block self-center!"
          />
          <a href="#" className="sticky top-0 bg-background z-10 block"></a>
          <h4 className="scroll-m-20 font-normal tracking-tight text-xl">
            {pageTitle}
          </h4>
        </div>
